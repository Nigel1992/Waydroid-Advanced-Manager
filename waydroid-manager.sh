...
accessibility_tools() {
    ...
    echo -e "${GREEN}└────────────────────────────────────────────┘${NC}"
    echo ""
    local lt_status="DISABLED"
    local hc_status="DISABLED"
    [ "${LARGE_TEXT:-0}" -eq 1 ] && lt_status="ENABLED"
    [ "${HIGH_CONTRAST:-0}" -eq 1 ] && hc_status="ENABLED"
    echo -e "  ${BOLD}1)${NC} 🔍 Toggle Large Text Mode    [${BOLD}${lt_status}${NC}]"
    echo -e "  ${BOLD}2)${NC} 🔊 Text-to-Speech (read status aloud)"
    echo -e "  ${BOLD}3)${NC} 🖥️  Toggle High-Contrast Mode [${BOLD}${hc_status}${NC}]"
